...
+(function (a) {
  (EditableSelect = function (b, c) {
    (this.options = c),
      (this.$select = a(b)),
      (this.$input = a('<input type="text" autocomplete="off">')),
      (this.$list = a('<ul class="es-list">')),
    ...
    (EditableSelect.prototype.filter = function () {
      ...
      this.$list.find("li").addClass("es-visible").show(),
        this.options.filter &&
          ...
          this.$list.find("li").length == b && this.hide());
    }),
    (EditableSelect.prototype.show = function () {
      if (
        ...
        var b = { default: "show", fade: "fadeIn", slide: "slideDown" },
          c = b[this.options.effects];
        this.utility.trigger("show"),
          this.$input.addClass("open"),
          this.$list[c](
            this.options.duration,
            a.proxy(this.utility.trigger, this.utility, "shown")
          );
      }
    }),
    (EditableSelect.prototype.hide = function () {
      var b = { default: "hide", fade: "fadeOut", slide: "slideUp" },
        c = b[this.options.effects];
      this.utility.trigger("hide"),
        this.$input.removeClass("open"),
        this.$list[c](
          this.options.duration,
          a.proxy(this.utility.trigger, this.utility, "hidden")
        );
    }),
    (EditableSelect.prototype.select = function (a) {
      this.$list.has(a) &&
        a.is("li.es-visible:not([disabled])") &&
        (this.$input.val(a.text()),
        this.options.filter && this.hide(),
        this.filter(),
        this.utility.trigger("select", a));
    }),
    (EditableSelect.prototype.add = function (b, c, d, e) {
      var f = a("<li>").html(b),
        g = a("<option>").text(b),
        h = this.$list.find("li").length;
      (c = isNaN(c) ? h : Math.min(Math.max(0, c), h)),
        0 == c
          ? (this.$list.prepend(f), this.$select.prepend(g))
          : (this.$list
              .find("li")
              .eq(c - 1)
              .after(f),
            this.$select
              ...
              .eq(c - 1)
              .after(g)),
        this.utility.setAttributes(f, d, e),
        this.utility.setAttributes(g, d, e),
        this.filter();
    }),
    (EditableSelect.prototype.remove = function (a) {
      var b = this.$list.find("li").length;
      (a = isNaN(a) ? b : Math.min(Math.max(0, a), b - 1)),
        this.$list.find("li").eq(a).remove(),
        this.$select.find("option").eq(a).remove(),
        this.filter();
    }),
    ...
    (EditableSelectUtility.prototype.initialize = function () {
      ...
      b.setAttributes(
        b.es.$input,
        b.es.$select[0].attributes,
        b.es.$select.data()
      ),
        b.es.$input.addClass("es-input").data("editable-select", b.es),
        b.es.$select.find("option").each(function (c, d) {
          var e = a(d).remove();
          b.es.add(e.text(), c, d.attributes, e.data()),
            e.attr("selected") && b.es.$input.val(e.text());
        }),
        b.es.filter();
    }),
    (EditableSelectUtility.prototype.initializeList = function () {
      ...
      b.es.$list
        .on("mousemove", "li:not([disabled])", function () {
          b.es.$list.find(".selected").removeClass("selected"),
    ...
    (EditableSelectUtility.prototype.initializeInput = function () {
      ...
      switch (this.es.options.trigger) {
        ...
        case "focus":
          b.es.$input
            .on("focus", a.proxy(b.es.show, b.es))
            .on("blur", a.proxy(b.es.hide, b.es));
          break;
        case "manual":
      }
      b.es.$input.on("input keydown", function (a) {
        switch (a.keyCode) {
          ...
          case 40:
            ...
            break;
          case 13:
            b.es.$list.is(":visible") &&
              (b.es.select(b.es.$list.find("li.selected")), a.preventDefault());
            break;
          case 9:
          case 27:
            b.es.hide();
        ...
        setTimeout(function () {
          ...
          if (c.length > 0) {
            ...
            g.addClass("selected"),
              f < e &&
                h < 0 &&
                b.es.$list.scrollTop(b.es.$list.scrollTop() + h),
              f > e &&
                h + g.outerHeight() > b.es.$list.outerHeight() &&
                b.es.$list.scrollTop(
                  b.es.$list.scrollTop() +
                    g.outerHeight() +
                    2 * (h - b.es.$list.outerHeight())
                );
          }
        });
    }),
    ...
    (EditableSelectUtility.prototype.trigger = function (a) {
      var b = Array.prototype.slice.call(arguments, 1),
        c = [a + ".editable-select"];
      c.push(b),
        this.es.$select.trigger.apply(this.es.$select, c),
        this.es.$input.trigger.apply(this.es.$input, c);
    }),
    (Plugin = function (b) {
      ...
      return this.each(function () {
        ...
        e || (e = new EditableSelect(this, f)),
          "string" == typeof b && e[b].apply(e, c);
      });
    }),
    (a.fn.editableSelect = Plugin),
    (a.fn.editableSelect.Constructor = EditableSelect);
})(jQuery);
