...
function report_UniqueCategories() {
  ...
  for (var i = 0; i < imgNames.length; i++) {
    ...
    for (var shape_i = 0; shape_i < image.shapes.length; shape_i++) {
      setData.add(
        (image.shapes[shape_i].category || "X") +
          "|" +
          (image.shapes[shape_i].label || "X")
      );
    }
  }
...
function saveAsDlibXML() {
  ...
  askFileName(
    Object.keys(labellingData).length + "_imglab.xml",
    function (fileName) {
...
function saveAsCOCO() {
  ...
  askFileName(
    Object.keys(labellingData).length + "_coco_imglab.json",
    function (fileName) {
...
function saveAsPascalVOC() {
  if (!imgSelected) {
    ...
    askFileName(
      Object.keys(labellingData[imgSelected.name].shapes.length).length +
        "_pvoc_imglab.xml",
      function (fileName) {
...
function saveAsYoloV5Pytorch() {
  ...
  for (const key in labellingData) {
    if (
      labellingData[key].shapes[0] &&
      labellingData[key].shapes[0].points.length > 0
    ) {
      totalLabbeledImages++;
    }
  }
  ...
  $.dialog({
    ...
    content: `<div class="col w-75 m-auto">
                    ...
                    <div class="row justify-content-between">
                        <input type="text" class="col" value="${Math.floor(
                          (totalLabbeledImages / 100) * 70
                        )}" onchange="javascript:validateImageSplit()" style="width: 20px;" placeholder=""> 
                        <input type="text" class="col" value="${Math.floor(
                          (totalLabbeledImages / 100) * 30
                        )}" onchange="javascript:validateImageSplit()" style="width: 20px;" placeholder=""> 
                        <input type="text" class="col" value="${Math.floor(
                          (totalLabbeledImages / 100) * 10
                        )}" onchange="javascript:validateImageSplit()" style="width: 20px;" placeholder=""> 
                    </div>
                </div>
...
function validateImageSplit() {
  ...
  var remainingImages =
    totalLabbeledImages - trainImages - testImages - validImages;
  if (remainingImages > 0) {
    document.getElementById(
      "splitMsg"
    ).innerText = `${remainingImages} Labelled Images remaining to split.`;
  }
  ...
  if (trainImages + testImages + validImages == totalLabbeledImages) {
    ...
    showSnackBar(
      `Total of ( Train+Test+Valid ) must be ${totalLabbeledImages}.`
    );
    return [0, 0, 0, false];
  }
}
...
function yoloDataRendering() {
  ...
  var timeStamp = `${curTimeStamp.getDate()}/ ${
    curTimeStamp.getMonth() + 1
  }/ ${curTimeStamp.getFullYear()}/
