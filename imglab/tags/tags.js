riot.tag2(
  ...
  'actionbar #actionbar,[data-is="actionbar"] #actionbar{ height: 40px; width: 100%; border: 1px solid grey; }',
  "",
  function (opts) {}
);
riot.tag2(
  "facepp",
  '<input onchange="{saveKey}" class="form-control" type="text" name="api_key" id="fpp_api_key" placeholder="api_key" riot-value="{pluginsStore.facepp.key}"> <input onchange="{saveSecret}" class="form-control" type="text" name="api_secret" id="fpp_api_secret" placeholder="api_secret" riot-value="{pluginsStore.facepp.secret}"> <br> <input onclick="{fetchFpp}" class="btn fppBtn" type="button" name="faceppBtn" id="faceppBtn" value="Plot with Face++">',
  'facepp .fppBtn,[data-is="facepp"] .fppBtn{ background: #17a2b8; width: 100%;color: white; }',
  "",
  function (opts) {
    ...
    this.fetchFpp = function () {
      if (
        !imgSelected.src ||
        !pluginsStore.facepp.key ||
        !pluginsStore.facepp.key
      ) {
        showSnackBar(
          "You need to load an image in workarea and input Face++ credential above ."
        );
      } else {
    ...
    function prepareFormData(url, imgdata) {
      ...
      if (url) {
        ...
        formData.append(
          "image_base64",
          imgdata.substr(imgdata.indexOf(";base64,") + 8)
        );
      }
    ...
    function fetchFromFpp(formData, fn) {
      $.ajax({
        ...
        error: function (err) {
          $("#info").text(
            "Error in connecting face++ API. Check console for more deetail"
          );
    ...
    function fppToImgLabObject(fppResponse) {
      ...
      for (var face_i in fppResponse.faces) {
        ...
        for (var a_i in attributes) {
          ...
          if (attribute.value) {
            labellingData[imgName].shapes[faceLable].attributes[
              attributes[a_i]
            ] = attribute.value;
          }
        }
  ...
  function (opts) {
    ...
    function deleteThumbnail(e) {
      ...
      for (var thumbnail_i in this.thumbnails) {
        if (
          this.thumbnails[thumbnail_i].name === $(thumbnail[0]).attr("title")
        ) {
...
riot.tag2(
  "menu",
  '<div class="dropdown" style="left: -50px"> <div class="dropbtn"><img src="img/icons/menu.svg" width="40px"></div> <div class="dropdown-content"> <a href="#"> <label class="btn-bs-file">Open <input id="browse" type="file" class="filebutton" accept=".fpp,.nimn,.xml,.json" onchange="{openFile}"> </label> </a> <a href="#" onclick="{saveFile}">Save</a> </div> </div>',
  "",
  "",
  function (opts) {
...
riot.tag2(
  "plugin-window",
  '<div id="plugin-window"> <div id="plugin-titlebar"> <span>{opts.plugin.title}</span> <button type="button" class="close" aria-label="Close" onclick="{closeme}"> <span aria-hidden="true" style="color: white;">&times;</span> </button> </div> <div id="plugin-content"></div> </div>',
  'plugin-window #plugin-titlebar,[data-is="plugin-window"] #plugin-titlebar{ background: grey; color: white; text-align: center; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; } plugin-window #plugin-window,[data-is="plugin-window"] #plugin-window{ border: 2px solid grey; top: 200px; left: 40vw; position: absolute; min-width: 200px; background: #c3bfbf; z-index: 10; } plugin-window #plugin-content,[data-is="plugin-window"] #plugin-content{ padding: 10px; }',
  "",
  function (opts) {
    this.on("mount", function () {
      $("#plugin-content").append(
        `<${opts.plugin.tagName}></${opts.plugin.tagName}>`
      );
...
riot.tag2(
  "plugins-menu",
  '<div class="dropdown" style="float:right;"> <div class="dropbtn"><img src="img/icons/clock.svg" width="35px"></div> <div class="dropdown-content"> <a href="#" each="{plugin in plugins}" onclick="{loadPlugin}">{plugin.title}</a> </div> </div>',
  "",
  "",
  function (opts) {
...
riot.tag2(
  "shortcuts",
  '<img src="img/icons/keyboard.svg" width="35px" onclick="{showkeymaps}">',
  "",
  "",
  function (opts) {
...
riot.tag2(
  "toolbox",
  '<div each="{tool,toolid in tools[opts.tools]}" id="{toolid}" class="tool-button" onclick="{selectme}"> <img class="tool-icon" riot-src="img/icons/{tool.icon}"> <div>{tool.title}</div> </div>',
  'toolbox .tool-button,[data-is="toolbox"] .tool-button{ text-align: center; margin: 14px 0px; font-size: 0.9em; } toolbox .tool-icon,[data-is="toolbox"] .tool-icon{ width: 40%; margin: 3px 0px; } toolbox .tool-selected,[data-is="toolbox"] .tool-selected{ background: coral; } toolbox .tool-button:not(.tool-selected):hover,[data-is="toolbox"] .tool-button:not(.tool-selected):hover{ background: grey; }',
  "",
  function (opts) {
    var tag = this;
...
riot.tag2(
  "workarea",
  '<div id="canvas-container"> <img id="img" riot-src="{opts.img.src}" width="{opts.img.size.width}" height="{opts.img.size.height}"> <div id="work-canvas" width="{opts.img.size.width}" height="{opts.img.size.height}"></div> <span id="tooltip-span"></span> </div>',
  'workarea #work-canvas,[data-is="workarea"] #work-canvas{ position: absolute; z-index: 1; } workarea #canvas-container,[data-is="workarea"] #canvas-container{ height: calc(100vh - 190px); display: block; overflow: auto; position: relative; }',
  "",
  function (opts) {
    ...
    this.on("mount", function () {
      myCanvas = new SVG("work-canvas").size(
        opts.img.size.width,
        opts.img.size.height
      );
      ...
      myCanvas.on("mousedown", function (event) {
        ...
        if (
          selectedTool &&
          selectedTool.type !== "point" &&
          !alreadyDrawing &&
          selectedTool.drawable
        ) {
